...
public class BoxTool {
    ...
    public BoxTool(HttpUtils httpUtils, SafeExecuteToolHandler safeExecuteToolHandler) {
    ...
    public Map<String,Object> createBox() {
        return safeExecuteToolHandler.mapTool(()->httpUtils.get("/box" ).block());
    }
//    删除沙盒
    @Tool(name = "delete_box",value = "删除沙盒")
    public Map<String, Object> deleteBox(@P("沙盒ID") String boxId){
        return safeExecuteToolHandler.mapTool(()->httpUtils.delete("/box/{box_id}", boxId).block());
    }
...
//    执行代码
    ...
    public Map<String,Object> executeCode(@P("沙盒ID")String boxId,@P("代码")Object code){
        ...
        return safeExecuteToolHandler.mapTool(()-> httpUtils.post("/execute/code",Map.of("box_id",boxId,"code",codeStr)).block());
    }
//    执行命令
    @Tool(name = "execute_cmd",value = "执行命令(Linux系统)")
    public Map<String,Object> executeCmd(@P("沙盒ID")String boxId,@P("命令")String cmd){
        return safeExecuteToolHandler.mapTool(()-> httpUtils.post("/execute/cmd",Map.of("box_id",boxId,"cmd",cmd)).block());
    }
}
